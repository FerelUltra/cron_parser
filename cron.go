...
func parsePart(part string, startValue, maxValue int) (string, error) {
	...
	if strings.Contains(part, "-") {
		...
		if err != nil {
			return "", fmt.Errorf("conversion error start_value: %v", err)
		}

		end, err := strconv.Atoi(hyphenSplits[1])
		if err != nil {
			return "", fmt.Errorf("conversion error end_value: %v", err)
		}
	...
	if strings.Contains(part, "/") {
		...
		if err != nil {
			return "", fmt.Errorf("conversion error step value: %v", err)
		}
